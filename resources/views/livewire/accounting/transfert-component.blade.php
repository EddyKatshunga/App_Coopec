{{-- resources/views/livewire/accounting/transfert-component.blade.php --}}
<div class="p-6 sm:p-8 bg-gray-50 min-h-screen">
    <div class="max-w-3xl mx-auto">
        <!-- En-tête avec infos Agence/Journée -->
        <div class="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
            <div>
                <h1 class="text-2xl font-bold text-slate-800 tracking-tight">Transfert de fonds</h1>
                <p class="text-sm text-slate-500">Mouvements entre la caisse et le compte bancaire</p>
            </div>
            ...
            <form wire:submit.prevent="save" class="p-6 md:p-8 space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="md:col-span-2">
                        <label class="block text-sm font-semibold text-slate-700 mb-2">Direction du transfert</label>
                        <div class="inline-flex p-1 bg-slate-100 rounded-xl w-full">
                            <button type="button" wire:click="$set('sens', 'caisse_vers_banque')" 
                                    class="flex-1 py-2 text-sm font-medium rounded-lg transition-all {{ $sens === 'caisse_vers_banque' ? 'bg-white shadow-sm text-indigo-600' : 'text-slate-500 hover:text-slate-700' }}">
                                Caisse → Banque
                            </button>
                            <button type="button" wire:click="$set('sens', 'banque_vers_caisse')" 
                                    class="flex-1 py-2 text-sm font-medium rounded-lg transition-all {{ $sens === 'banque_vers_caisse' ? 'bg-white shadow-sm text-indigo-600' : 'text-slate-500 hover:text-slate-700' }}">
                                Banque → Caisse
                            </button>
                        </div>
                        @error('sens') <span class="text-rose-500 text-xs mt-1 italic">{{ $message }}</span> @enderror
                    </div>
                ...
                <div class="pt-4">
                    <button type="submit" wire:loading.attr="disabled"
                        ...
                        <span wire:loading.remove wire:target="save">Effectuer le transfert</span>
