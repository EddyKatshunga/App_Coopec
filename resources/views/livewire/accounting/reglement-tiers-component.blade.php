{{-- resources/views/livewire/accounting/reglement-tiers-component.blade.php --}}
<div class="p-6 sm:p-8 bg-gray-50 min-h-screen">
    <div class="max-w-3xl mx-auto">
        <!-- En-tête avec infos Agence/Journée -->
        <div class="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
            <div>
                <h1 class="text-2xl font-bold text-slate-800 tracking-tight">Règlement des tiers</h1>
                <p class="text-sm text-slate-500">Paiement de dettes ou encaissement de créances</p>
            </div>
            ...
            <form wire:submit.prevent="save" class="p-6 md:p-8 space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="md:col-span-2">
                        <label class="block text-sm font-semibold text-slate-700 mb-2">Type d'opération</label>
                        <div class="inline-flex p-1 bg-slate-100 rounded-xl w-full">
                            <button type="button" wire:click="$set('type', 'dette')" 
                                    class="flex-1 py-2 text-sm font-medium rounded-lg transition-all {{ $type === 'dette' ? 'bg-white shadow-sm text-indigo-600' : 'text-slate-500 hover:text-slate-700' }}">
                                Paiement de dette
                            </button>
                            <button type="button" wire:click="$set('type', 'creance')" 
                                    class="flex-1 py-2 text-sm font-medium rounded-lg transition-all {{ $type === 'creance' ? 'bg-white shadow-sm text-indigo-600' : 'text-slate-500 hover:text-slate-700' }}">
                                Encaissement de créance
                            </button>
                        </div>
                        @error('type') <span class="text-rose-500 text-xs mt-1 italic">{{ $message }}</span> @enderror
                    </div>

                    <!-- Montant -->
                    <div>
                        <label for="montant" class="block text-sm font-semibold text-slate-700 mb-2">Montant</label>
                        <div class="relative">
                    ...
                    <div class="md:col-span-2">
                        <label for="reference" class="block text-sm font-semibold text-slate-700 mb-2">Référence / Justificatif (Optionnel)</label>
                        <input type="text" wire:model="reference" id="reference" placeholder="Ex: Facture F2025-001, Créance client #1234"
                               class="w-full rounded-xl border-slate-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 transition-all py-3 px-4">
                        @error('reference') <span class="text-rose-500 text-xs mt-1 italic">{{ $message }}</span> @enderror
                    </div>
                </div>
